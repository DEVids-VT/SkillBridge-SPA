    ...
    <div className={cn(cards.base, 'flex flex-col h-full')}>
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <Badge className={cn('text-xs font-semibold', getEventTypeColor(type))}>
      ...
      <div className={cn(cards.body, 'flex-1 flex flex-col')}>
        ...
        <p className="text-sm text-gray-600 font-medium mb-1">By {organizer}</p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {tags.slice(0, 3).map((tag, index) => (
            <span key={index} className={cn(components.tag, 'bg-gray-100 text-gray-700')}>
              {tag}
            </span>
          ))}
          {tags.length > 3 && (
